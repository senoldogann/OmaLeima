...
import {
  invokeUpdateClubDepartmentTagRpcAsync,
...
import {
  ...
  parseClubDepartmentTagUpdatePayloadOrThrow,
  ...
  try {
    ...
    if (userId === null) {
      return NextResponse.json(
        {
          message: "Sign in again before updating an official department tag.",
    ...
    const rateLimitResponse = await enforceDashboardMutationRateLimitAsync(userId, "club-department-tag-update");

    if (rateLimitResponse !== null) {
      return rateLimitResponse;
    }

    const body = parseClubDepartmentTagUpdatePayloadOrThrow(
      (await request.json()) as Record<string, string>
    );
    const result = await invokeUpdateClubDepartmentTagRpcAsync(supabase, {
      actorUserId: userId,
