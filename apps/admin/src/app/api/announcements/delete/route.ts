...
export async function POST(request: Request) {
  try {
    ...
    if (userId === null) {
      return NextResponse.json({ message: "Sign in again before deleting announcements.", status: "AUTH_REQUIRED" }, { status: 401 });
    }

    const rateLimitResponse = await enforceDashboardMutationRateLimitAsync(userId, "announcement-delete");
    ...
    const result = await deleteAnnouncementAsync(supabase, {
      announcementId,
      clubId,
    });
    ...
    console.error("[announcement-delete] failed", {
      message: error instanceof Error ? error.message : "Unknown announcement delete route error.",
    });
