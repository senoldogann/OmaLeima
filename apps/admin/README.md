...
`npm run bootstrap:pilot-operator-accounts` uses the same hosted Supabase CLI auth path, creates placeholder admin/organizer/scanner password accounts with random passwords, archives the old fixture operator users, and writes the resulting credentials to `/Users/dogan/Desktop/OmaLeima-pilot-operator-credentials.txt` on this workstation.
`npm run audit:pilot-secret-hygiene` is the local follow-up gate for that Desktop credential file. It checks strict file mode (`600`), parses the current admin/organizer/scanner entries, rejects duplicate or weak passwords, and verifies the required GitHub secret names are still present without printing any secret values.
`npm run run:pilot-final-dry-run` reads that same Desktop credential file, signs the hosted admin/organizer/scanner accounts in with the current password set, and verifies organizer/scanner active membership shape before a private pilot.
...
`npm run smoke:pilot-secret-hygiene-audit` is the deterministic fixture-backed smoke for the local secret/password hygiene audit. It verifies one permissive/weak failure path and one strict-ready success path without touching the real Desktop credential file.
