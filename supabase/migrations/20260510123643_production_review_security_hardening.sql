...
begin
  ...
  if v_scanner_profile.status <> 'SUSPENDED'
    or v_scanner_profile.primary_role <> 'STUDENT'
    or v_scanner_profile.email not like 'anonymous-%@scanner.omaleima.local'
    or exists (
      select 1
      from public.business_staff bs
      where bs.user_id = p_scanner_user_id
        and bs.status = 'ACTIVE'
    )
    or exists (
      select 1
      from public.business_scanner_devices bsd
      where bsd.scanner_user_id = p_scanner_user_id
        and bsd.status = 'ACTIVE'
    )
  then
    return jsonb_build_object('status', 'ACTOR_NOT_ALLOWED');
  end if;
